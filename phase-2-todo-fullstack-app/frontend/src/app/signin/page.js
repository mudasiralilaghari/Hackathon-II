...
export default function SigninPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('Signin response:', data); // Debug logging

      // Store the token in localStorage
      if (data.access_token) {
        localStorage.setItem('access_token', data.access_token);
        console.log('Token stored in localStorage:', data.access_token.substring(0, 20) + '...'); // Log first 20 chars
      } else {
      ...
      console.error('Signin error:', err); // Debug logging
